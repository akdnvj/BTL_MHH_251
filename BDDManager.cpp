...
// // =========================================================
// // Helper Class: PetriNet Logic (Sao chép và tinh chỉnh từ main.cpp)
// // Được giấu trong file .cpp để giữ header BDDManager sạch sẽ
// // =========================================================
...
//     // Xây dựng quan hệ chuyển trạng thái (Logic từ main.cpp)
//     DdNode* build_transition_relation(CuddMgr mgr, const std::vector<DdNode*>& vars, const std::vector<DdNode*>& primed_vars) {
//         int n_vars = (int)vars.size();
//         DdNode* R = Cudd_Not(Cudd_ReadOne(mgr)); Cudd_Ref(R); // False

//         for (const auto& t_pair : transitions) {
//             const std::string& t_id = t_pair.first;
//             DdNode* T = Cudd_ReadOne(mgr); Cudd_Ref(T); // True
...
//                 DdNode* change = nullptr;
//                 if (is_post) { change = primed_vars[i]; Cudd_Ref(change); }
//                 else if (is_pre) { change = Cudd_Not(primed_vars[i]); Cudd_Ref(change); }
//                 else {
//                     DdNode* c1 = Cudd_bddAnd(mgr, vars[i], primed_vars[i]); Cudd_Ref(c1);
//                     DdNode* c2 = Cudd_bddAnd(mgr, Cudd_Not(vars[i]), Cudd_Not(primed_vars[i])); Cudd_Ref(c2);
//                     change = Cudd_bddOr(mgr, c1, c2); Cudd_Ref(change);
...
//     // 2. Load PNML sử dụng Helper
//     PetriNetHelper pn;
//     if (!pn.load_pnml(pnml_file)) {
//         // Fallback nếu lỗi: Tạo BDD False
//         root->node = Cudd_Not(Cudd_ReadOne(mgr)); 
//         Cudd_Ref(root->node);
//         return;
//     }

//     // 3. Lưu thông tin biến
//     this->ordered_place_ids = pn.ordered_place_ids;
//     int nvars = (int)ordered_place_ids.size();
    
//     // Tạo biến CUDD
//     std::vector<DdNode*> primed_vars(nvars);
//     for (int i = 0; i < 2 * nvars; ++i) Cudd_bddIthVar(mgr, i); // Cấp phát slot
...
//     // 4. Tính toán Reachability (Symbolic)
//     // Initial State
//     Marking initM = pn.get_initial_marking_vector();
//     DdNode* S = Cudd_ReadOne(mgr); Cudd_Ref(S);
//     for (int i = 0; i < nvars; ++i) {
...
//     std::vector<DdNode*> compose_vec(2 * nvars);
//     for (int i = 0; i < nvars; ++i) {
//         compose_vec[i] = vars[i]; // dummy (unused)
//         compose_vec[nvars + i] = vars[i]; // replace primed with unprimed
//     }
...
//     // Lưu kết quả vào root
//     root->node = S; // S đã được Ref
//     std::cout << "BDDManager initialized. Reachable set built (" << Cudd_DagSize(root->node) << " nodes).\n";
// }
...
// bool BDDManager::internalCheck(BDDNode* node, const std::vector<int>& inputs) {
//     // Sử dụng Cudd_Eval để kiểm tra giá trị của hàm BDD tại điểm 'inputs'
//     // inputs cần khớp với mảng biến int* mà Cudd_Eval yêu cầu
//     if (!node || !node->node) return false;

//     // Chuyển vector marking thành mảng int inputs cho tất cả các biến CUDD
//     // CUDD quản lý biến theo chỉ số (0..N-1), ta chỉ cần map marking vào đúng vị trí
//     int n = Cudd_ReadSize(mgr);
//     int* input_array = new int[n];
//     memset(input_array, 0, sizeof(int) * n);

//     for (size_t i = 0; i < inputs.size() && i < vars.size(); ++i) {
//         // Giả định biến i tương ứng với chỉ số i trong CUDD (do cách ta init)
//         input_array[i] = inputs[i]; 
//     }

//     DdNode* res = Cudd_Eval(mgr, node->node, input_array);
//     bool isReachable = !Cudd_IsComplement(res); // True(1) là node constant 1 (không phải complement của 1)
...
// bool BDDManager::checkReachability(const std::vector<int>& marking) {
//     if (marking.size() != ordered_place_ids.size()) {
//         std::cerr << "Error: Marking size mismatch.\n";
//         return false;
//     }

//     // Cách 1: Sử dụng giao (Intersection) giữa Cube của marking và tập Reachable
//     DdNode* mCube = markingToCube(marking);
//     DdNode* intersection = Cudd_bddAnd(mgr, root->node, mCube); Cudd_Ref(intersection);
    
//     bool reachable = (intersection != Cudd_Not(Cudd_ReadOne(mgr))); // Khác False (Zero)
...
//     // Cách 2: (Thay thế bằng internalCheck nếu muốn dùng Cudd_Eval)
//     // return internalCheck(root, marking);
// }
